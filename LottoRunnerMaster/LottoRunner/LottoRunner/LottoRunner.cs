...
namespace LottoRunner
{
    class LottoRunner
    {
        static void Main(string[] args)
        {
            ...
            if (!WriteWithConfirmation(Labels.DrawBallQuestion))
            {
                userTicket = LottoService.LottoBallSpinner.DrawBalls();
            }
            else
            {
                userTicket = GetUserEnteredTicket();
            }

            Write(Labels.YourNumbers, userTicket.ToString());

            LottoTicket lottoBallSet; 
            // for testing
            // = new LottoTicket(new int[] { userTicket.WhiteBall1, userTicket.WhiteBall2, userTicket.WhiteBall3, userTicket.WhiteBall4, userTicket.WhiteBall5 }, userTicket.RedBall);
            ...
            do
            {
                lottoBallSet = LottoService.LottoBallSpinner.DrawBalls();
                runs++;

                var result = lottoBallSet.Compare(userTicket);

                if (!CompareAndPrintResults(result)) //, 2, true)
                {
                    if (runs % 10000000 == 0)
                        Write(Labels.RunsAndBallSet, FormatInt(runs), lottoBallSet.ToString()); 
                }
                else
                {
                    break;
